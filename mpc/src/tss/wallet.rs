...
mod tests {
    ...
    #[test]
    fn test_tss_wallet_creation() {
        let wallet = TSSWallet::new(SolanaNetwork::Devnet);
        assert_eq!(wallet.get_current_network(), &SolanaNetwork::Devnet);
    }
    ...
    fn test_aggregate_keys() {
        ...
        let aggregate = wallet.aggregate_keys(keys.clone(), Some(2));

        assert_eq!(aggregate.participant_keys, keys);
        assert_eq!(aggregate.threshold, 2);
    }

    #[test]
    fn test_network_switching() {
        let mut wallet = TSSWallet::new(SolanaNetwork::Devnet);
        assert_eq!(wallet.get_current_network(), &SolanaNetwork::Devnet);

        wallet.switch_network(SolanaNetwork::MainnetBeta);
        assert_eq!(wallet.get_current_network(), &SolanaNetwork::MainnetBeta);
    }
    ...
    fn test_public_key_validation() {
        ...
        assert!(wallet.validate_public_key(valid_key).is_ok());

        // Invalid public key
        let invalid_key = "invalid_key";
        assert!(wallet.validate_public_key(invalid_key).is_err());
    }
}
