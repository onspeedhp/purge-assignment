...
mod tests {
    ...
    #[test]
    fn test_tss_keypair_creation() {
        let keypair = TSSKeypair::new();
        assert_eq!(keypair.public_key.to_bytes().len(), 32);
        assert_eq!(keypair.secret_key.len(), 32);
    }

    #[test]
    fn test_aggregate_wallet_creation() {
        let keys = vec![
            Pubkey::new_unique(),
            Pubkey::new_unique(),
            Pubkey::new_unique(),
        ];
        let wallet = AggregateWallet::new(keys.clone(), Some(2));

        assert_eq!(wallet.participant_keys, keys);
        assert_eq!(wallet.threshold, 2);
        assert_eq!(wallet.aggregated_public_key.to_bytes().len(), 32);
    }

    #[test]
    fn test_network_urls() {
        assert_eq!(
            SolanaNetwork::MainnetBeta.rpc_url(),
    ...
    #[test]
    fn test_enhanced_tss_keypair() {
        let keypair = EnhancedTSSKeypair::new(3);
        assert_eq!(keypair.public_key.to_bytes().len(), 32);
        assert_eq!(keypair.secret_key.len(), 32);
        assert_eq!(keypair.threshold, 3);
        assert_eq!(keypair.key_hash.len(), 32);
    }
    ...
    fn test_tss_config() {
        ...
        let config = TSSConfig::new(participants.clone(), 2);
        assert_eq!(config.threshold, 2);
        assert_eq!(config.total_participants, 3);
        assert_eq!(config.participants, participants);
        assert!(config.has_sufficient_participants(2));
        assert!(!config.has_sufficient_participants(1));
    }
    ...
    fn test_mpc_session() {
        ...
        let mut session = MPCSession::new("test_session".to_string(), participants.clone(), 2);

        assert_eq!(session.session_id, "test_session");
        assert_eq!(session.participants, participants);
        assert_eq!(session.threshold, 2);
        assert_eq!(session.status, MPCSessionStatus::Initializing);
        assert!(!session.is_ready());

        session.update_status(MPCSessionStatus::Ready);
        assert!(session.is_ready());
    }

    #[test]
    fn test_aggregated_threshold_signature() {
        ...
        let aggregated_sig = AggregatedThresholdSignature {
        ...
        assert!(aggregated_sig.verify(message));
    }
}
