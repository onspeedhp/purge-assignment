...
impl Database {
    ...
    pub async fn store_key_share(&self, user_id: &str, public_key: &str, private_key: &str) -> Result<KeyShare, AppError> {
    ...
    pub async fn get_key_share_by_user_id(&self, user_id: &str) -> Result<Option<KeyShare>, AppError> {
        let row = sqlx::query(
    ...
    pub async fn get_key_share_by_public_key(&self, public_key: &str) -> Result<Option<KeyShare>, AppError> {
        let row = sqlx::query(
    ...
    pub async fn get_key_shares_by_user_ids(&self, user_ids: &[String]) -> Result<HashMap<String, KeyShare>, AppError> {
    ...
    /// Update key share
    pub async fn update_key_share(&self, user_id: &str, public_key: &str, private_key: &str) -> Result<KeyShare, AppError> {
        let now = Utc::now();
        ...
        self.get_key_share_by_user_id(user_id).await?
    ...
    pub async fn list_key_shares(&self) -> Result<Vec<KeyShare>, AppError> {
        ...
        let key_shares = rows.into_iter().map(|row| KeyShare {
            id: row.get("id"),
            user_id: row.get("user_id"),
            public_key: row.get("public_key"),
            private_key: row.get("private_key"),
...
mod tests {
    ...
    async fn test_database_operations() {
        ...
        let key_share = db.store_key_share("user1", "pubkey1", "privkey1").await.unwrap();
        ...
        let updated = db.update_key_share("user1", "pubkey1_updated", "privkey1_updated").await.unwrap();
